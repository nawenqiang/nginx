...
static void* ngx_http_myfilter_create_conf(ngx_conf_t *cf) {
    ngx_http_myfilter_conf_t* mycf = (ngx_http_myfilter_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_myfilter_conf_t)); 
    if (mycf == NULL) {
        return NULL;
    }
...
static ngx_int_t ngx_http_myfilter_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    ngx_http_myfilter_ctx_t *ctx = ngx_http_get_module_ctx(r, ngx_http_myfilter_module); 
    ...
    */
    ...
    /*从请求的内存池中生成
    ngx_chain_t链表，将刚分配的ngx_buf_t设置到
    buf成员中，并将它添加到原先待发送的HTTP包体前面
    */
